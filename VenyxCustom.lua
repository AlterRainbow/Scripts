...
local themes = {
    ...
    glow = Color3.fromRGB(232, 232, 232),
    topbar = Color3.fromRGB(38, 38, 38),
    sectionsBackground = Color3.fromRGB(4, 56, 61),
    itemsBackground = Color3.fromRGB(10, 9, 9),
    text = Color3.fromRGB(250, 250, 250)
}

do
    function functions:Create(instance, properties, children)
        local object = Instance.new(instance)

        for property, propertyValue in pairs(properties or {}) do
            object[property] = propertyValue

            if typeof(propertyValue) == "Color3" then
                local theme = functions:Find(themes, propertyValue)

                if theme then
                    objects[theme] = objects[theme] or {}
                    objects[theme][property] = objects[theme][property] or setmetatable({}, {_mode = "k"})

                    table.insert(objects[theme][property], object)
                end
            end
        end
    ...
    function functions:Tween(instance, properties, duration, ...)
		tweenS:Create(instance, tweenInfo(duration, ...), properties):Play()
	end
    ...
    function functions:Find(table, value)
    ...
    function functions:Sort(pattern, values)
		local new = {}
    ...
    function functions:InitializeKeybind()
		self.keybinds = {}
		self.ended = {}
    ...
    function functions:BindToKey(key, callback)
    ...
    function functions:KeyPressed()
		local key = inputS.InputBegan:Wait()
		
		while key.UserInputType ~= Enum.UserInputType.Keyboard	 do
			key = inputS.InputBegan:Wait()
		end
...
do
    ...
    function library.new(title)
        local container = functions:Create("ScreenGui", {
			Name = title,
			Parent = game.CoreGui
		}, {
			functions:Create("ImageLabel", {
				Name = "Main",
				BackgroundTransparency = 1,
				Position = UDim2.new(0.5, 0, 0.5, 0),
				Size = UDim2.new(0, 504, 0, 378),
				...
				functions:Create("ImageLabel", {
					...
					Size = UDim2.new(0.068, 126, 1, -38),
					ZIndex = 3,
					Image = "rbxassetid://5012534273",
					ImageColor3 = themes.itemsBackground,
					ScaleType = Enum.ScaleType.Slice,
					SliceCenter = Rect.new(4, 4, 296, 296)
				}, {
					functions:Create("ScrollingFrame", {
        ...
        functions:InitializeKeybind()
		functions:DraggingEnabled(container.Main.Topbar, container.Main)
    ...
    function page.new(library, title)
		...
		local container = functions:Create("ScrollingFrame", {
			Name = title,
			...
			Size = UDim2.new(0.947, -142, 1, -56),
			CanvasSize = UDim2.new(0, 0, 0, 466),
			ScrollBarThickness = 3,
			ScrollBarImageColor3 = themes.itemsBackground,
			Visible = false
		}, {
			functions:Create("UIListLayout", {
				SortOrder = Enum.SortOrder.LayoutOrder,
    ...
    function section.new(page, title)
		local container = functions:Create("ImageLabel", {
			Name = title,
    ...
    function library:setTheme(theme, color3)
		...
		for propertyI, objects in pairs(objects[theme]) do
			for i, object in pairs(objects) do
				if not object.Parent or (object.Name == "Button" and object.Parent.Name == "ColorPicker") then
					objects[i] = nil
				else
					object[propertyI] = color3
				end
			end
		end
	end
    ...
    function library:toggle()
		...
		if self.position then
			...
			functions:Tween(topbar, {Size = UDim2.new(1, 0, 0, 38)}, 0.2)
			wait(0.2)
			...
			functions:Tween(topbar, {Size = UDim2.new(1, 0, 1, 0)}, 0.2)
			wait(0.2)
    ...
    function library:Notification(title, text, callback)
		if self.activeNotification then
			self.activeNotification = self.activeNotification()
		end
		
		local notification = functions:Create("ImageLabel", {
			Name = "Notification",
        ...
        functions:DraggingEnabled(notification)
		...
		functions:Tween(notification, {Size = UDim2.new(0, textSize.X + 70, 0, 60)}, 0.2)
		wait(0.2)
    ...
    function section:Button(title, callback)
		local button = functions:Create("ImageButton", {
			Name = "Button",
			Parent = self.container,
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		button.MouseButton1Click:Connect(function()
			...
			debounce = true
			text.TextSize = 0
			functions:Tween(button.Title, {TextSize = 14}, 0.2)
			
			wait(0.2)
			functions:Tween(button.Title, {TextSize = 12}, 0.2)
	...
	function section:Toggle(title, default, callback)
		local toggle = functions:Create("ImageButton", {
			Name = "Toggle",
			Parent = self.container,
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
	...
	function section:Textbox(title, default, callback)
		local textbox = functions:Create("ImageButton", {
			Name = "Textbox",
			Parent = self.container,
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		textbox.MouseButton1Click:Connect(function()
			...
			functions:Tween(textbox.Button, {
	...
	--[[function section:TextboxNumerical(title, default, callback)
		local textbox = functions:Create("ImageButton", {
			Name = "Textbox",
			Parent = self.container,
			BackgroundTransparency = 1,
			...
			functions:Create("ImageButton", {
				Name = "Negative",
				BackgroundTransparency = 1,
				Position = UDim2.new(0.662, 0, 0.233, 0),
				Size = UDim2.new(0, 16, 0, 16),
				ZIndex = 2,
				HoverImage = "rbxassetid://6871369854"
				Image = "rbxassetid://5028857472",
				ImageColor3 = themes.sectionsBackground,
				ScaleType = Enum.ScaleType.Slice,
				SliceCenter = Rect.new(2, 2, 298, 298)
		...
		local button = textbox.Button
		local input = button.Textbox
		local negative = button.Negative
	...
	function section:Keybind(title, default, callback, changedCallback)
		local keybind = functions:Create("ImageButton", {
			Name = "Keybind",
			Parent = self.container,
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		keybind.MouseButton1Click:Connect(function()
			...
			if text.Text == "None" then
				...
				local key = functions:KeyPressed()
	...
	function section:ColorPicker(title, default, callback)
		local colorpicker = functions:Create("ImageButton", {
			Name = "ColorPicker",
			Parent = self.container,
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		local tab = functions:Create("ImageLabel", {
			Name = "ColorPicker",
			...
			functions:Create("Frame", {
				...
				functions:Create("ImageButton", {
					Name = "Canvas",
					BackgroundTransparency = 1,
					BorderColor3 = themes.sectionsBackground,
				...
				functions:Create("ImageButton", {
					...
					functions:Create("UIGradient", { -- rainbow canvas
				...
				functions:Create("Frame", {
					...
					functions:Create("ImageLabel", {
						...
						functions:Create("TextBox", {
							...
							ZIndex = 2,
							Font = Enum.Font.Gotham,
							PlaceholderColor3 = themes.itemsBackground,
							Text = "255",
							TextColor3 = themes.text,
							TextSize = 10.000
						})
		...
		canvas.MouseButton1Down:Connect(function()
			...
			while draggingCanvas do
				...
				functions:Tween(canvas.Cursor, {Position = UDim2.new(sat, 0, 1 - brightness, 0)}, 0.1) -- overwrite
		...
		color.MouseButton1Down:Connect(function()
			...
			while draggingColor do
				...
				local x = hue
				self:updateColorPicker(colorpicker, nil, {hue, sat, brightness})
				functions:Tween(tab.Container.Color.Select, {Position = UDim2.new(x, 0, 0, 0)}, 0.1)
		...
		animate = function(visible, overwrite)
			...
			if visible then
				...
				functions:Tween(tab, {Size = UDim2.new(0, 162, 0, 169)}, 0.2)
				...
				functions:Tween(tab, {Size = UDim2.new(0, 0, 0, 0)}, 0.2)
				tab.ClipsDescendants = true
	...
	function section:Slider(title, default, min, max, callback)
		local slider = functions:Create("ImageButton", {
			Name = "Slider",
			Parent = self.container,
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
	...
	function section:Dropdown(title, list, callback)
		local dropdown = functions:Create("Frame", {
			Name = "Dropdown",
			Parent = self.container,
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 30),
		...
		search.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
			if focused then
				local list = functions:Sort(search.TextBox.Text, list)
		...
		if toggle then
			...
			if sectionsRequired < 0 then
				for i = existingSections, #page.sections + 1, -1 do
					...
					functions:Tween(section, {ImageTransparency = 1}, 0.1)
				end
			end
			...
			if sectionsRequired > 0 then
				for i = existingSections + 1, #page.sections do
					...
					functions:Tween(section, {ImageTransparency = 0}, 0.05)
				end
			end
			...
			for i, section in pairs(page.sections) do
				...
				functions:Tween(section.container.Title, {TextTransparency = 0}, 0.1)
				section:Resize(true)
			...
			for i, section in pairs(page.sections) do	
				functions:Tween(section.container.Parent, {Size = UDim2.new(1, -10, 0, 28)}, 0.1)
				functions:Tween(section.container.Title, {TextTransparency = 1}, 0.1)
			end
	...
	function page:Resize(scroll)
		...
		if scroll then
			functions:Tween(self.container, {CanvasPosition = Vector2.new(0, self.lastPosition or 0)}, 0.2)
		end
	end
	...
	function section:Resize(smooth)
		...
		if smooth then
			functions:Tween(self.container.Parent, {Size = UDim2.new(1, -10, 0, size)}, 0.05)
		else
	...
	function section:updateKeybind(keybind, title, key)
		...
		if key then
			self.binds[keybind].connection = functions:BindToKey(key, bind.callback)
			text.Text = key.Name
		else
			text.Text = "None"
		end
	...
	function section:updateColorPicker(colorpicker, title, color)
		...
		functions:Tween(colorpicker.Button, {ImageColor3 = color3}, 0.5)
		functions:Tween(tab.Container.Color.Select, {Position = UDim2.new(hue, 0, 0, 0)}, 0.1)
		
		functions:Tween(tab.Container.Canvas, {ImageColor3 = Color3.fromHSV(hue, 1, 1)}, 0.5)
		functions:Tween(tab.Container.Canvas.Cursor, {Position = UDim2.new(sat, 0, 1 - brightness)}, 0.5)
	...
	function section:updateSlider(slider, title, value, min, max, lvalue)
		...
		local percent = (playerMouse.X - bar.AbsolutePosition.X) / bar.AbsoluteSize.X
		...
		functions:Tween(bar.Fill, {Size = UDim2.new(percent, 0, 1, 0)}, 0.1)
		
		if value ~= lvalue and slider.ImageTransparency == 0 then
			functions:Pop(slider, 10)
		end
	...
	function section:updateDropdown(dropdown, title, list, callback)
		...
		functions:Tween(dropdown, {Size = UDim2.new(1, 0, 0, (entries == 0 and 30) or math.clamp(entries, 0, 3) * 34 + 38)}, 0.3)
		functions:Tween(dropdown.Search.Button, {Rotation = list and 180 or 0}, 0.3)
